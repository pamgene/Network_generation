...
pNetworkScore <- function(score_obs, score_obs_inv, uka_cell_all, condition, nsample, nPerms = DFT.PERMS,
  ...
  network_sig_score <- max(sum(score_perm <= score_obs) / nPerms, 1 / nPerms)
  network_inv_sig_score <- max(sum(score_perm_inv >= score_obs_inv) / nPerms, 1 / nPerms)
...
clean_uka_to_kinograte <- function(uka, cs = F) {
  ...
  uka_clean <- uka %>%
    ...
    dplyr::select("Sample", "Kinase Name", all_of(stat_col), all_of(finalscore_col)) %>%
    dplyr::rename(
...
golden_results_to_barplot <- function(results_list, perccutoff = 0.7) {
  ...
  results_df <- lapply(results_list, function(x) {
    ...
    bind_rows() %>%
    mutate(path = sub(".*PTKSTK\\/(.*)\\/thrs_.*", "\\1", path))
